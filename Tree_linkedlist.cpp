...
void create_binarytr e e(nod e *root)
{
char option;
if( root != NULL )
{
...
{
root- >lchild = getnode();
create_binarytre e(root- >lchild);
}
else
{
root- >lchild = NULL;
...
printf("\n Node %s has Right SubTree(Y/N) ",root- >dat a);
fflush(stdin);
scanf("%c",&option);
if( option = = 'Y' || option = = 'y')
{
root- >rchild = getnode();
create_binarytre e(root- >rchild);
}
else
{
root- >rchild = NULL;
...
void make_Queue(nod e *root,int parent)
{
if(root != NULL)
{
node_ctr + + ;
Q[parent] = root;
make_Queue(root- >lchild,pare n t*2 + 1);
make_Queue(root- >rchild,par e n t*2 + 2);
}
}
delete_node(nod e *root, int parent)
{
int index = 0;
if(root = = NULL)
printf("\n Empty TREE ");
